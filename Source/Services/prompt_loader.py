...
class PromptLoader:
    ...
    def preload_all_prompts(self) -> None:
        ...
        prompt_files = {
            'subject_detection': 'subject_detection_prompt',
            'syllabus_generation': 'syllabus_prompt',
            'file_summarization': 'file_summarization_prompt',
            'section_summarization': 'section_summarization_prompt',
            'course_summarization': 'course_summarization_prompt'
        }
    ...
    def get_prompt(self, prompt_type: str, section: str = "system", reload: bool = False, **kwargs) -> str:
        ...
        prompt_files = {
            ...
            'section_summary': 'section_summarization_prompt',
            'course_summary': 'course_summarization_prompt'
        }
...
if __name__ == "__main__":
    ...
    print("Testing Subject Detection Prompt:")
    system_prompt = loader.get_prompt("subject_detection", "system")
    print(f"System: {system_prompt if system_prompt else 'NOT FOUND'}...")

    print("\nTesting Syllabus Generation Prompt:")
    syllabus_prompt = loader.get_prompt("syllabus_generation", "system",
                                        subject_name="מתמטיקה בדידה", subject_type="מתמטי")
    print(f"Syllabus: {syllabus_prompt if syllabus_prompt else 'NOT FOUND'}...")

    print("\nTesting Section Summarization Prompt:")
    section_prompt = loader.get_prompt("section_summarization", "system",
                                       subject_name="מתמטיקה בדידה", subject_type="מתמטי")
    print(f"Section: {section_prompt if section_prompt else 'NOT FOUND'}...")

    print("\nTesting Section Summarization Prompt (Humanities):")
    section_prompt_hum = loader.get_prompt("section_summarization", "system",
                                           subject_name="היסטוריה", subject_type="הומני")
    print(f"Section (Humanities): {section_prompt_hum if section_prompt_hum else 'NOT FOUND'}...")

    print("\nTesting Section Summarization Prompt (General Math):")
    section_prompt_gen = loader.get_prompt("section_summarization", "system",
                                           subject_type="מתמטי")
    print(f"Section (General Math): {section_prompt_gen if section_prompt_gen else 'NOT FOUND'}...")

    print("\n" + "=" * 80)
    print("Testing USER Section Summarization Prompt (Math with Subject Name):")
    user_section_prompt = loader.get_prompt("section_summarization", "user",
                                            subject_name="מתמטיקה בדידה", subject_type="מתמטי")
    print(f"User Section: {user_section_prompt if user_section_prompt else 'NOT FOUND'}")

    print("\n" + "=" * 80)
    print("Testing USER Section Summarization Prompt (Humanities with Subject Name):")
    user_section_prompt_hum = loader.get_prompt("section_summarization", "user",
                                                subject_name="היסטוריה", subject_type="הומני")
    print(f"User Section (Humanities): {user_section_prompt_hum if user_section_prompt_hum else 'NOT FOUND'}")

    print("\n" + "=" * 80)
    print("Testing USER Section Summarization Prompt (General Math):")
    user_section_prompt_gen = loader.get_prompt("section_summarization", "user",
                                                subject_type="מתמטי")
    print(f"User Section (General Math): {user_section_prompt_gen if user_section_prompt_gen else 'NOT FOUND'}")
