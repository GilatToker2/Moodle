...
class AdvancedVideoSearch:
    ...
    def semantic_search_with_rewrites(self, query: str, top_k: int = 5) -> List[Dict]:
        """חיפוש סמנטי עם query rewrites"""
        logger.info(f"🔍 חיפוש סמנטי עם rewrites: '{query}'")
        print("=" * 60)
        ...
        try:
            ...
            print(f"✅ נמצאו {len(docs)} תוצאות סמנטיות עם rewrites:")
            ...
            print(f"❌ שגיאה בחיפוש סמנטי עם rewrites: {e}")
            logger.error(f"Error in semantic search with rewrites: {e}")
    ...
    def search_in_video(self, query: str, video_id: str, top_k: int = 5) -> List[Dict]:
        """חיפוש שאלה ספציפית בתוך וידאו ספציפי"""
        logger.info(f"🔍 חיפוש '{query}' בתוך Video ID: '{video_id}'")
        print("=" * 60)
        ...
        try:
            ...
            # יצירת embedding לשאלה לחיפוש היברידי
            query_vector = self.generate_query_embedding(query)

            if query_vector:
                # חיפוש היברידי בתוך הוידאו הספציפי
                results = self.search_client.search(
                    search_text=query,
                    filter=f"video_id eq '{escaped_video_id}'",
                    vector_queries=[VectorizedQuery(
                ...
                # חיפוש טקסטואלי בלבד אם אין embedding
                results = self.search_client.search(
                    search_text=query,
                    filter=f"video_id eq '{escaped_video_id}'",
                    select=[
            ...
            if not docs:
                print(f"❌ לא נמצאו תוצאות עבור '{query}' בוידאו {video_id}")
                return []

            print(f"✅ נמצאו {len(docs)} תוצאות עבור '{query}' בוידאו {video_id} (מתוך {total_count} צ'אנקים בוידאו):")
            ...
            print(f"❌ שגיאה בחיפוש בתוך וידאו: {e}")
            logger.error(f"Error in search within video: {e}")
            return []
    ...
    def compare_search_methods(self, query: str, top_k: int = 3) -> Dict:
        ...
        # 4. חיפוש סמנטי עם rewrites
        print("\n4️⃣ חיפוש סמנטי עם rewrites:")
        print("-" * 40)
        results['semantic'] = self.semantic_search_with_rewrites(query, top_k)
    ...
    def get_video_summary(self, video_id: str) -> Dict:
        """קבלת סיכום קצר של וידאו ספציפי"""
        ...
        try:
            ...
            results = self.search_client.search(
                ...
                select=["video_name", "start_seconds"],
                order_by=["start_seconds asc"],
    ...
    try:
        ...
        if status.get("sample_doc"):
            ...
            if sample_video_id:
                ...
                # דוגמה לחיפוש בתוך וידאו ספציפי
                print(f"\n{'=' * 80}")
                print(f"🎯 דוגמה: חיפוש שאלה בתוך וידאו ספציפי")
                print(f"{'=' * 80}")

                # נשתמש בשאלה מהדמו לחיפוש בתוך הוידאו הספציפי
                demo_query = "מה זה פסוק לוגי"
                print(f"חיפוש השאלה '{demo_query}' בתוך הוידאו {sample_video_id}:")
                search_system.search_in_video(demo_query, sample_video_id, top_k=3)

        print(f"\n🎉 דמו הושלם בהצלחה!")
        print("💡 ניתן להשתמש במערכת עם שאלות נוספות או לחפש לפי Video ID ספציפי")
        print("🔍 השתמש בפונקציה search_in_video() לחיפוש שאלות בתוך וידאו ספציפי")
